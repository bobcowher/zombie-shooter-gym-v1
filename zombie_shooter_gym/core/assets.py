...
import pygame
import math
from zombie_shooter_gym.core.util import check_collision
...
import zombie_shooter_gym.resources.images as images_pkg


def load_image(filename):
    """Load an image from the package resources."""
    img_file = files(images_pkg) / filename
    return pygame.image.load(str(img_file))
...
class Player:
    ...
    def __init__(self, world_width, world_height, walls) -> None:
        ...
        for direction in ('up', 'down', 'left', 'right'):
            image = load_image(f'player_{direction}.png')
...
class Zombie:
    def __init__(self, world_width, world_height, size=50, speed=1):
        ...
        for direction in ('up', 'down', 'left', 'right'):
            image = load_image(f'zombie_{direction}.png')
            self.images[direction] = pygame.transform.scale(image, (self.size, self.size))
...
class TreasureChest:
    def __init__(self, x, y):
        self.closed_image = load_image("chest_closed.png").convert_alpha()
        self.opened_image = load_image("chest_opened.png").convert_alpha()
...
class HealthDrop:
    def __init__(self, x, y):
        self.image = load_image("heart.png").convert_alpha()  # Load heart image
